...
<h2>3강</h2>
<strong>1.폼 데이터와 <form>태그</strong>
<p>폼 데이터는 <form></form> 태그에 실려 전송되는 데이터를 말한다. <form> 태그에는 전송할 데이터를 어디에 어떻게 보낼지도 설정하는데, 어디에 보낼지는 action 속성으로,  어떻게 보낼지는 method 속성으로 설정한다.</p>
2.DTO
